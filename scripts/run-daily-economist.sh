...
echo "[$(date --iso-8601=seconds)] Starting Economist sync"
exec 9>"$LOCK_FILE"
if ! flock -n 9; then
...
if git status --porcelain | grep -q .; then
  ...
  git fetch duoread source main
  current_branch="$(git rev-parse --abbrev-ref HEAD)"
  if [ "$current_branch" != "source" ]; then
    git switch source 2>/dev/null || git switch -c source --track duoread/source
  fi
  git pull --ff-only duoread source
fi
...
node scripts/sync-latest-economist.mjs --publish true

echo "[$(date --iso-8601=seconds)] Economist sync finished"
