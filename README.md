...
`bilingual.json` 是网站的数据源。每个自然段都有同一个 `id` 下的 `en` 和 `zh` 字段。网页支持“穿插语言”和“一种语言”两种阅读模式；轻点正文会切换当前语言显示，并保持被点击自然段在屏幕里的位置，滑动阅读时不会切换。网站会用 cookie 记住当前选择的杂志和文章。移动端使用文章下拉选择，桌面端使用左侧文章列表。
